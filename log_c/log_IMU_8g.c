...
int main(int argc, char *argv[])
{
  ...
  double accel_LSB = 8.0/32768.0;	// 1.0 / 8192.0; // = 4.0/32768.0
  double gyro_LSB = 500.0/32768.0; //1.0 / 131.0;   // = 250.0/32768.0
  ...
  if (fd != -1)
  {
    ...
    {
      gettimeofday(&start_tv, NULL);
      while (1) //just print data in terminal
      {
        read(fd, buf, 1);
        printf("c=%x %c\n", buf[0], buf[0]);
        if (buf[0] == 0x47)
        {
          read(fd, buf, 12);
#ifdef NORMALIZED_LH
          Ax = (double)((int16_t)((buf[1] << 8) | buf[0])) * (8.0 / 32768.0);
          Ay = (double)((int16_t)((buf[3] << 8) | buf[2])) * (8.0 / 32768.0);
          Az = (double)((int16_t)((buf[5] << 8) | buf[4])) * (8.0 / 32768.0);
          Gx = (double)((int16_t)((buf[7] << 8) | buf[6])) * (500.0 / 32768.0);
          Gy = (double)((int16_t)((buf[9] << 8) | buf[8])) * (500.0 / 32768.0);
          Gz = (double)((int16_t)((buf[11] << 8) | buf[10])) * (500.0 / 32768.0);
    ...
    printf("could not connect to arduino. Leaving the program.\n");
